...
public class JwtAuthService
{
    ...
    public string GenerateToken(UserEmployee userAfterVerifyPass)
    {
        ...
        //Claim = cặp key–value mô tả danh tính user.
        //Các claims mặc định trong token:
        //"UserName" → tên đăng nhập.
        //"Email" → email user.
        //sub(Subject) → định danh chính(ở đây cũng dùng username).
        //jti(JWT ID) → số ngẫu nhiên duy nhất, giúp chống việc replay token.
        ...
        //Truy vấn DB(UserRoles + Role) để lấy danh sách quyền của user.
        //Với mỗi role tìm được → thêm vào claims:
        //Nhờ đó, token có thể được dùng với[Authorize(Roles = "Admin")] trong controller.
...
//        var claims = new List<Claim>
//    {
//        new Claim("UserId", userAfterVerifyPass.Id.ToString()),
//        new Claim("UserName", userAfterVerifyPass.Username),
//        new Claim(JwtRegisteredClaimNames.Sub, userAfterVerifyPass.Username),
...
//        // 🧩 Thêm danh sách quyền (Role)
//        var roles = _context.RoleUsers
//            .Include(r => r.Role)
//            .Where(r => r.UserId == userAfterVerifyPass.Id)
